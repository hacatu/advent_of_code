...
def add_snums(a, b):
	res = [deepcopy(a), deepcopy(b)]
	reduce_snum(res)
	return res

with open(sys.argv[1], "r") as f:
	snums = [literal_eval(line) for line in f.readlines() if line.strip()]
	print(max(magnitude(add_snums(a, b)) for (a, b) in permutations(snums, 2)))
#	print(max(permutations(snums, 2), key=lambda d: magnitude(add_snums(*d))))
